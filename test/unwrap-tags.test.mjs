...
const { default: unwrapTags } = await loadModule(`
  export { default } from './src/commands/unwrap-tags';
`);
...
function applyOpsToText(text, doc, ops) {
  const edits = ops
...
test('unwrapTags: 单行标签移除', async () => {
  ...
  setConfig({});
  try {
    const doc = makeDocument('<span>text</span>', join(ws, 'u1.html'), 'html');
...
test('unwrapTags: 嵌套标签与多光标同时解构', async () => {
  ...
  try {
    ...
    const doc = makeDocument(text, join(ws, 'u3.html'), 'html');
...
test('unwrapTags: 多光标命中同一标签对去重', async () => {
  ...
  try {
    ...
    const doc = makeDocument(text, join(ws, 'u4.html'), 'html');
    const editor = editorWith(doc, [
...
test('unwrapTags: 嵌套多行标签解构重叠区间合并', async () => {
  ...
  try {
    const text = '<div>\n  <span>x</span>\n</div>';
    const doc = makeDocument(text, join(ws, 'u5.html'), 'html');
    const editor = editorWith(doc, [
