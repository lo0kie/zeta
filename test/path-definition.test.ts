import { extractImportString, PathDefinitionProvider } from '@/providers/path-definition';
...
import { cleanup, makeDocument, makeWorkspace, normSep, setConfig } from './helpers';

const provider = new PathDefinitionProvider();

test('extractImportString: 精确提取带引号的导入路径及 Range', () => {
  const line = `  import { x } from "@alias/components/Button";  `;
  const charIndex = line.indexOf('alias'); // 模拟光标落在路径中
  const result = extractImportString(0, line, charIndex);

  assert.ok(result, '应提取到导入字符串');
  assert.equal(result.rawPath, '@alias/components/Button');
  assert.equal(result.stringRange.start.character, line.indexOf('"'));
  assert.equal(result.stringRange.end.character, line.lastIndexOf('"') + 1);

  // 光标在引号上也算命中（char <= end）
  const onQuote = extractImportString(0, line, line.indexOf('"'));
  assert.ok(onQuote, '光标在开引号上应命中');

  // 负向：光标在 import 关键字上（字符串之外）返回 undefined
  const outOfRange = extractImportString(0, line, line.indexOf('import'));
  assert.equal(outOfRange, undefined);
});

function firstFsPath(loc: vscode.LocationLink[] | null | undefined): string | undefined {
  if (!loc || loc.length === 0) return undefined;
  return loc[0].targetUri.fsPath;
}

// 返回所有命中的文件 fsPath（单结果也归一为数组）；provider 返回 LocationLink[]
function allFsPaths(loc: vscode.LocationLink[] | null | undefined): string[] {
  if (!loc) return [];
  return loc.map(l => l.targetUri.fsPath);
}
...
test('父级相对路径 ../foo 跳转到上级目录文件', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 19));
    assert.equal(firstFsPath(loc), join(ws, 'foo.ts'));
  } finally {
    cleanup(ws);
  }
...
test('省略 ./ 前缀的相对子路径 src/utils/foo 仍可跳转', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 25));
...
test('@/ 别名经 tsconfig paths 跳转到 src/components/Button.tsx', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 30));
...
test('@/ 别名下的 .vue 组件：显式带扩展名与省略扩展名都能跳转到 .vue 文件', async () => {
  ...
  try {
    ...
    const loc1 = await provider.provideDefinition(
      doc1,
      new vscode.Position(0, line1.indexOf('BaseSegmentedControl') + 5)
    );
    assert.ok(loc1 !== undefined, '显式扩展名应返回结果');
    assert.ok(
      allFsPaths(loc1)
        .map(normSep)
        .includes(normSep(join(ws, 'src', 'components', 'BaseSegmentedControl.vue'))),
      '显式 .vue 应命中组件文件'
    );
    ...
    const loc2 = await provider.provideDefinition(
      doc2,
      new vscode.Position(0, line2.indexOf('BaseSegmentedControl') + 5)
    );
    assert.ok(
      allFsPaths(loc2)
        .map(normSep)
...
test('绝对工作区路径 /src/components 跳转到目录的 index 文件', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 24));
...
test('非导入位置的普通字符串不触发跳转', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 15));
    assert.equal(loc, undefined);
  } finally {
    cleanup(ws);
  }
...
test('光标落在字符串之外（如 import 关键字上）不触发跳转', async () => {
  ...
  try {
    ...
    writeFileSync(join(ws, 'foo.ts'), 'export const foo = 1;');
    const doc = makeDocument(`import foo from './foo';\n`, join(ws, 'main.ts'), 'typescript');
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 3));
    assert.equal(loc, undefined);
  } finally {
    cleanup(ws);
  }
...
test('含点的文件名 my.component 不被误拆（my.component.ts 可被找到）', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 20));
...
test('CSS Modules：@/assets/tokens.module 同时有 .ts 与 .less，两者都返回（不依赖内置 TS 服务）', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, line.indexOf('tokens.module') + 5));
    assert.ok(Array.isArray(loc), '应返回 Location 数组');
    const paths = allFsPaths(loc).map(normSep);
...
test('返回 LocationLink，originSelectionRange 覆盖整个导入字符串（含引号）', async () => {
  ...
  try {
    ...
    const doc = makeDocument(line, join(ws, 'src', 'main.ts'), 'typescript');
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, line.indexOf('BaseSegmentedControl') + 5));
    ...
    assert.ok(link.originSelectionRange, '应有 originSelectionRange');
...
test('originSelectionRange 对相对路径字符串同样覆盖整个字符串', async () => {
  ...
  try {
    ...
    assert.ok(Array.isArray(loc) && loc.length > 0, '应返回结果数组');
    const link = loc[0];
    assert.ok(link.originSelectionRange, '相对路径也应有 originSelectionRange');
    assert.equal(
      line.slice(link.originSelectionRange!.start.character, link.originSelectionRange!.end.character),
      "'./foo'",
      '相对路径也应整串下划线'
...
test('无工作区（单文件打开）+ 仅 package.json：@/ 别名仍按项目根 src 兜底解析', async () => {
  ...
  try {
    ...
    const paths = allFsPaths(loc).map(normSep);
    assert.ok(
      paths.includes(normSep(join(proj, 'src', 'assets', 'tokens.module.less'))),
      '应兜底命中 src/assets/tokens.module.less'
    );
  } finally {
    cleanup(ws);
...
test('自定义 zeta.path.extensions：只探测配置的后缀', async () => {
  ...
  try {
    ...
    const loc = await provider.provideDefinition(doc, new vscode.Position(0, 12));
    assert.ok(loc, '应返回结果');
    const paths = allFsPaths(loc).map(normSep);
